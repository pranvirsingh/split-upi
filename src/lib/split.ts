 ...
 * Uses integer paise arithmetic to avoid floating-point rounding errors.
 * The returned amounts are in rupees and always sum EXACTLY to totalAmount.
 *
 * splitAmount(4500, 1999) -> [1999, 1999, 502]
 * splitAmount(2000, 1999) -> [1999, 1]
 * splitAmount(5997, 1999) -> [1999, 1999, 1999]
 * splitAmount(1000, 1999) -> [1000]
 */
export function splitAmount(totalAmount: number, maxPerPayment: number): number[] {
  ...
  // Convert back to rupees, exact to 2 decimals
  return parts.map((p) => Math.round(p) / 100);
}

/** Sum helper (paise-safe) for verification */
